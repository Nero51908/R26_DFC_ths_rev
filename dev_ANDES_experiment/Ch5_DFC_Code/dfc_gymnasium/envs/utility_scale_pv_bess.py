...
class UtilityScalePVBESS(gym.Env):
  ...
  def _pvbess_dynamics(self, action: np.ndarray):
    ...
    # which dispatches on config.reward_fn: "firm" (default redesigned reward, tunables in
    # config.reward_params) or "n3" (old symmetric baseline). Never re-inline reward per env.
    Pnet = Ppv - Pb
    actual_c = 1 - Ppv / Pm
