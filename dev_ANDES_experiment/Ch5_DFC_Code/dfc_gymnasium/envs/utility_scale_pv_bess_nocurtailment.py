...
class UtilityScalePVBESSnC(gym.Env):
  ...
  def __init__(self, 
    ...
    super(UtilityScalePVBESSnC, self).__init__()
  ...
  def step(self, action: np.ndarray):
    ...
    # truncation due to time limit will not happen in this environment, truncate=False
    truncated = False
  ...
  def _pvbess_dynamics(self, action: np.ndarray):
    ...
    c = 0 # make curtailment ratio zero for this case of the evn is only using BESS for net power control
    Ppv = Pm # set PV output as Pm is the implementation of c = 0
    Pb, next_soc = self._bess_dynamics(Ppv, Pdfc)

    # calculate reward via the shared per-step entry point (helper_fns.env_step_reward);
    # dispatches on config.reward_fn ("firm" default / "n3" baseline). Same reward path as the
    # curtailment env — never re-inline reward per env.
    Pnet = Ppv - Pb
    actual_c = 1 - Ppv / Pm   # = 0 here (c forced to 0; BESS-only control)
    reward = hlp.env_step_reward(Pnet, Pdfc, actual_c)
